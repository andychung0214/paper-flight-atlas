...
若瀏覽器控制能力可用，請在 390px、768px 與 1440px 寬度檢查首頁、圖鑑、篩選、主題切換、收藏、8 個教學頁、步驟上一則／下一則、未知路由回復、鍵盤焦點、減少動畫與重新載入後狀態。若執行環境無法建立瀏覽器連線，改以靜態伺服器、DOM／CSS 契約與自動測試完成等價檢查，並在驗證報告明確記錄未完成真實視窗驗證；不得將等價檢查宣稱為瀏覽器驗證。記錄任何修正並重新執行 `npm test`。

...
- 四十張專屬折前／折後 SVG 摺紙示意圖，支援安全文字轉義、未知鍵值安全備援與放大 dialog。
- 圖解以移動紙面、摺線、方向箭頭與對齊點四個符號說明摺法；意義不只依賴顏色傳達。
...
直接開啟 `tests/browser-test.html`。這是僅在瀏覽器直接執行的測試，測試頁會顯示測試摘要與每項結果，目前涵蓋：資料完整性、路由、儲存失效、四十個唯一圖解鍵值與折前／折後 SVG、安全轉義與安全備援、四個主要畫面、入口 HTML/SEO 契約、收藏、篩選、主題、焦點、Skip Link、圖例、響應式圖解、dialog 與取消操作。
